...
**Part A：硬性规则检查（主对话完成，不依赖 subagent）**

使用 Read 工具读取文章，逐条检查以下硬性规则。发现任何问题直接修复，无需等待 subagent。

| 检查项 | 方法 |
|---|---|
| 字数范围 | Read 后估算正文字数 |
| 标点禁令 | 目视检查，无冒号、破折号、双引号 |
| 套话禁令 | 目视检查，禁用列表中的词汇 |
| 空泛工具名 | 目视检查 |
| 教科书开头 | 目视检查 |
| 表格限制 | 目视检查 |
...
| 专有名词 | 目视检查 |
| 引号格式 | 目视检查 |
| 固定结尾 | 检查文末是否有公众号引导和动图 |
| 配图完整性 | 检查所有图片引用是否可访问 |

**Part B：活人感终审（subagent 执行）**

调用 subagent 只执行「活人感终审」和「风格一致性」检查。subagent 只能使用 Read 工具读取文章，不能使用 Bash 工具（环境权限可能受限）。

根据两部分自检结果完成最终修改。将终稿更新到文件中，附上自检报告。同时提供三个标题推荐。
...
以下清单在 Step 9 Part A 由主对话逐条核实。任何一条未通过都必须修改后再提交：
